...
def main(data):
    ...
    part_names = {}
    for node in nodes:
        part_name = get_part_name(int(node.name))
        if part_name not in part_names.keys():
            part_names[part_name] = node.total_quantity
        else:
            part_names[part_name] += node.total_quantity

    part_df = pd.DataFrame(columns=['Part Name', 'Total Quantity'])
    part_df['Part Name'] = part_names.keys()
    part_df['Total Quantity'] = part_names.values()
